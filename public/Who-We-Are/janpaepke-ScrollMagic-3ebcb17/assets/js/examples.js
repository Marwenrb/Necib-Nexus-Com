...
function showCode($elem) {
  ...
  var $code = $('<div>')
    ...
    $ln = $('<div>')
      ...
      .wrapInner('<pre>'),
    $close = $('<div>').attr('id', 'close').addClass('button'),
    $select = $('<div>')
      .attr('id', 'select')
      .addClass('button')
      .text('select all'),
    $codewrap = $('<div>').addClass('codewrap').append($ln).append($code)
...
$(document).on('mousedown', '.slider, .move', function (e) {
  if (e.which === 1) {
    ...
    if ($this.is('.slider') || e.target == this) {
      // only the element itself,  not the children, unless its the slider
      e.stopPropagation()
