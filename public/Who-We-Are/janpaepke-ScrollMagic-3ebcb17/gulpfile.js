...
var args = yargs
  ...
  .describe(
    'd',
    'Generate new docs, optionally supplying output folder [default folder: ./"' +
      config.dirs.defaultDocsOutput +
      '"]'
  )
  .alias('d', 'doc')
  .default('d', false)
  ...
  .describe(
    'debug',
    "Enters debug mode: Allows 'debugger' statements to remain in the code during compilation."
  )
  .default('debug', false)
  .help('?')
  // examples
  ...
  .example(
    '$0 --doc=newdocs',
    'build and generate new docs into folder "newdocs"'
  )
  .example(
    '$0 --bump=patch',
    'build and update version number from to 2.1.1 to 2.1.2'
  ).argv
...
var summary = function () {
  ...
  return (
    gulp
      .src(options.folderOut + '/' + options.subfolder.uncompressed + '/*.js')
      .pipe(
        size({ showFiles: true, gzip: true, title: 'Main Lib uncompressed' })
      ) &&
    gulp
      .src(
        options.folderOut +
          '/' +
          options.subfolder.uncompressed +
          '/plugins/*.js'
      )
      .pipe(
        size({ showFiles: false, gzip: true, title: 'Plugins uncompressed' })
      ) &&
    gulp
      .src(options.folderOut + '/' + options.subfolder.minified + '/*.js')
      .pipe(
        size({ showFiles: true, gzip: true, title: 'Main Lib minified' })
      ) &&
    gulp
      .src(
        options.folderOut + '/' + options.subfolder.minified + '/plugins/*.js'
      )
      ...
      jeditor(
        {
          ...
          lastupdate:
            ...
            '-' +
            ('0' + (options.date.getMonth() + 1)).slice(-2) +
            '-' +
            ('0' + options.date.getDate()).slice(-2),
        },
        beautifyOptions
      )
    )
      ...
      replace({
        patterns: [
          ...
          },
          {
            // cdnjs url
            match:
              /(cdnjs.cloudflare.com\/ajax\/libs\/ScrollMagic\/)\d+\.\d+\.\d+(\-\w+)?(\/)/gi,
            replacement: '$1' + options.version + '$3',
          },
        ],
      })
    ...
    for (var methodname in pluginInfo.plugins[classname]) {
      pluginWarnings.push(
        warn
          ...
          .replace(/%PLUGIN%/g, pluginInfo.plugins[classname][methodname])
      )
    }
  }
  return (
    gulp
      .src(config.files, { base: config.dirs.source })
      // .pipe(plumber())
        ...
        replace({
          patterns: [
            ...
            },
            {
              ...
              match:
                /\/\/ \(BUILD\) - REMOVE IN MINIFY - START[^]*?\/\/ \(BUILD\) - REMOVE IN MINIFY - END/gm,
              replacement: '',
            },
          ],
        })
...
var copyStaticDocfiles = function () {
  return gulp
    .src('dev/docs/static/**/*.*', { base: process.cwd() + '/dev/docs/static' })
...
var compileDocs = function () {
  return gulp.src('dev/src/**/*.js', { base: process.cwd() + '/dev/src' }).pipe(
    jsdoc({
