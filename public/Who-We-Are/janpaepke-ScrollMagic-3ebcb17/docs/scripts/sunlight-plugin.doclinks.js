...
;(function (sunlight, document, undefined) {
  ...
  var supportedLanguages = {
    php: {
      function: function (word) {
        return 'http://php.net/' + word
      },
      languageConstruct: function (word) {
        return 'http://php.net/' + word
      },
    },
    ...
    perl: {
      function: function (word) {
        return 'http://perldoc.perl.org/functions/' + word + '.html'
      },
    },

    lua: {
      function: function (word) {
        return 'http://www.lua.org/manual/5.1/manual.html#pdf-' + word
      },
    },
  }
  ...
  function createLink(transformUrl) {
    return function (context) {
      ...
      link.className =
        context.options.classPrefix + context.tokens[context.index].name
      link.setAttribute(
        'href',
        transformUrl(context.tokens[context.index].value)
      )
