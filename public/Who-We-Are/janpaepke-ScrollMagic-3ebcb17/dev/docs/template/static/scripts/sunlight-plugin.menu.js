...
;(function (sunlight, document, undefined) {
  ...
  sunlight.bind('afterHighlightNode', function (context) {
    ...
    if (
      ...
      sunlight.util.getComputedStyle(context.node, 'display') !== 'block'
    ) {
      return
    }
    ...
    collapseLink.onclick = (function () {
      var originalHeight = sunlight.util.getComputedStyle(
          context.codeContainer,
          'height'
        ),
        originalOverflow = sunlight.util.getComputedStyle(
          context.codeContainer,
          'overflowY'
        )
      ...
      return function () {
        var needsToExpand =
          sunlight.util.getComputedStyle(context.codeContainer, 'height') !==
          originalHeight

        this.replaceChild(
          document.createTextNode(needsToExpand ? mDash : '+'),
          this.firstChild
        )
        this.setAttribute(
          'title',
          (needsToExpand ? 'collapse' : 'expand') + ' clode block'
        )
        context.codeContainer.style.height = needsToExpand
          ? originalHeight
          : '0px'
        context.codeContainer.style.overflowY = needsToExpand
          ? originalOverflow
          : 'hidden'
      ...
      return function () {
        ...
        if (textarea) {
          ...
          textarea.style.width =
            parseInt(sunlight.util.getComputedStyle(context.node, 'width')) -
            5 +
            'px' //IE, Safari and Chrome can't handle the actual width
          textarea.style.height = sunlight.util.getComputedStyle(
            context.node,
            'height'
          )
