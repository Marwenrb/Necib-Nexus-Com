      ...
      toBeOfType: function (util, customEqualityTesters) {
        return {
          compare: function (obj, objType) {
            ...
            if (result.pass) {
              result.message =
                'Expected ' + obj + " not to be of type '" + objType + "'"
            } else {
              result.message =
                'Expected ' +
                obj +
        ...
        return {
          compare: function (method, obj) {
            ...
            try {
              ...
              e.message +=
                ", while testing method '" +
                method +
                "' to be chainable when passing a parameter"
              throw e
            }
            if (result.pass) {
              result.message =
                "Expected method '" +
                method +
                "' not to be chainable when used as setter"
            } else {
              result.message =
                "Expected method '" +
                method +
                "' to be chainable when used as setter"
            }
            return result
          },
        }
          ...
          compare: function (method, obj) {
            ...
            if (result.pass) {
              result.message =
                "Expected method '" +
                method +
                "' not to be a getter and return parent"
            } else {
              result.message =
                "Expected method '" +
                method +
                "' to be a getter and return something other than parent"
            }
            return result
          },
        }
