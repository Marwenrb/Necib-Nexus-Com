...
async function generatePngFiles() {
  ...
  if (!fs.existsSync(SVG_PATH)) {
    console.error(
      colors.red('❌ Error: favicon.svg not found in public directory')
    )
    process.exit(1)
  }
    ...
    await Promise.all(
      sizes.map(async (sizeConfig) => {
        ...
        try {
          ...
          console.error(
            colors.red(`❌ Error generating ${sizeConfig.name}:`),
            error.message
          )
        }
      })
...
async function generateFaviconIco() {
  ...
  try {
    ...
    if (pngPaths.length === 0) {
      console.error(
        colors.red('❌ Error: No PNG files found for favicon.ico generation')
      )
      return
    }
...
async function updateConfigFiles() {
  ...
  try {
    ...
    if (fs.existsSync(browserconfigPath)) {
      ...
      browserconfig = browserconfig.replace(
        /<TileColor>#[a-fA-F0-9]{6}<\/TileColor>/,
        `<TileColor>${BRAND_PRIMARY}</TileColor>`
      )
    ...
    console.error(
      colors.red('❌ Error updating browserconfig.xml:'),
      error.message
    )
  }
  ...
  try {
    ...
    if (fs.existsSync(webmanifestPath)) {
      ...
      webmanifest = webmanifest.replace(
        /"theme_color":\s*"#[a-fA-F0-9]{6}"/,
        `"theme_color": "${BRAND_PRIMARY}"`
      )
      webmanifest = webmanifest.replace(
        /"background_color":\s*"#[a-fA-F0-9]{6}"/,
    ...
    console.error(
      colors.red('❌ Error updating site.webmanifest:'),
      error.message
    )
  }
}
...
async function generateFavicons() {
  console.log(
    colors.blue.bold(
      '🚀 Starting premium favicon generation for Necib Nexus...'
    )
  )
  ...
  try {
    ...
    if (!fs.existsSync(path.join(__dirname, '../node_modules/sharp'))) {
      ...
      require('child_process').execSync(
        'npm install --save-dev sharp svg-to-img png-to-ico colors',
        {
          stdio: 'inherit',
        }
      )
    ...
    console.log(
      colors.green.bold(
        '✅ All favicon assets have been generated successfully!'
      )
    )
    console.log(
      colors.blue.bold('🌟 Your premium UI/UX favicons are ready to use.')
    )
    console.log(
      colors.blue(
        '💡 Tip: Test your favicons on different devices and browsers to ensure optimal display.'
      )
    )
  } catch (error) {
    console.error(
      colors.red('❌ Error during favicon generation:'),
      error.message
    )
  }
}
